        ...
        lfs_off_t off, void *buffer, lfs_size_t size) 
{
    ...
    {
        offset = (block * c->block_size) + off;
        err = esp_partition_read(efs->partition, offset, buffer, size);
    } 
    else if (efs->media == ESP_LITTLEFS_MEDIA_SDMMC)
    {
        offset  = block * (efs->cfg.block_size / efs->cfg.read_size);
        offset += off / efs->cfg.read_size,
        err     = sdmmc_read_sectors(efs->card, buffer, offset, size / efs->cfg.read_size);
    }
